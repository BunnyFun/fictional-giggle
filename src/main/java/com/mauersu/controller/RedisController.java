...
public class RedisController extends RedisApplication implements Constant {
    ...
    public Object stringList(HttpServletRequest request, HttpServletResponse response,
        ...
        String queryByKeyPrefixs = StringUtil.getParameterByDefault(request, "queryByKeyPrefixs",
            EMPTY_STRING);

        String[] keyPrefixs = request.getParameterValues("keyPrefixs");

        Pagination pagination = stringListPagination(request, queryKey, queryKey_ch, queryValue,
            queryByKeyPrefixs);

        logCurrentTime("viewService.getRedisKeys start");
        Set<RKey> redisKeys = viewService.getRedisKeys(pagination, serverName, dbIndex, keyPrefixs,
            queryKey, queryValue);
    ...
    private Pagination stringListPagination(HttpServletRequest request, String queryKey,
        String queryKey_ch, String queryValue, String queryByKeyPrefixs) {
        Pagination pagination = getPagination(request);
        String url = "?" + "queryKey=" + queryKey + "&queryKey_ch=" + queryKey_ch + "&queryValue="
            + queryValue;
    ...
    private Pagination getPagination(HttpServletRequest request) {
        ...
        String num_display_entries = StringUtil.getParameterByDefault(request,
            "num_display_entries", "3");
        String visit_page = StringUtil.getParameterByDefault(request, "visit_page", "0");
        String num_edge_entries = StringUtil.getParameterByDefault(request, "num_edge_entries",
            "2");
        ...
        String prev_show_always = StringUtil.getParameterByDefault(request, "prev_show_always",
            "true");
        String next_show_always = StringUtil.getParameterByDefault(request, "next_show_always",
            "true");
    ...
    public Object getKV(HttpServletRequest request, HttpServletResponse response,
        ...
        WorkcenterResult result = (WorkcenterResult) redisService.getKV(serverName, dbIndex,
            dataType, key);
